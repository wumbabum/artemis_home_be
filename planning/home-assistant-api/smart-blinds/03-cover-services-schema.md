...
The response body shape on success/failure has not yet been
captured against a working device — pending Z-Wave radio recovery,
see `known-blockers.md`. Per HA convention and the existing prior
research, success returns an empty array `[]`; that needs
re-verification.
